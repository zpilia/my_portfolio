...
const Hobbies = () => {
    ...
    const { title, hobby1, hobby2, translations: translationsJson } = texts.hobbies;

    const baseTranslations = translationsJson.map(item => ({
        text: item.text,
        flag: item.flag === "japan.svg" ? japanFlag :
            item.flag === "korea.svg" ? koreaFlag :
                null
    }));
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setSlideIndex(prev => {
                if (prev >= baseTranslations.length) {
                    return 1;
                }
                return prev + 1;
            });
